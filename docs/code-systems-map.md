...
For deeper implementation notes on the highest-priority gameplay systems, see [`important-systems.md`](important-systems.md). For eudplib trigger/CUnit/TrgUnit implementation notes, see [`eudplib-trigger-notes.md`](eudplib-trigger-notes.md). For lessons from K's prior advanced EPS survival map, see [`prior-map-eps-lessons.md`](prior-map-eps-lessons.md).
